...
class CalculateObject:
    def __init__(self, params):
        ...
        self.part_quantities = params["part_quantities"]
        self.part_lengths = params["part_lengths"]
        self.part_names = params["part_names"]
        self.stock_length = params["stock_length"]
        self.left_waste = params["left_waste"]
        self.right_waste = params["right_waste"]
        self.spacing = params["spacing"]
        self.max_parts_per_nest = params["max_parts_per_nest"]
        self.max_containers = params["max_containers"]
    ...
    def column_gen(self, bandwidth, max_parts_per_nest, part_quantities, parts_sublist, limit_iterations):
        ...
        for i in range(self.num_parts):
            self.patterns[i, i] = min(self.part_quantities[i],
        ...
        while self.iteration_count < self.max_iterations:
            ...
            if self.find_cycling(ip_best_history):
                ...
                if limit_iterations == 1:
                    if self.max_iterations == 100000:  # TODO recode since this condition is only met on first loop
    ...
    def length_nest_pro_calculate(self):
        ...
        self.num_parts = len(self.part_lengths)

        # Set precision for printing
        np.set_printoptions(precision=3)
        # Remove any parts where the part quantity is 0 (remove entries from selfpart_names,
        # selfpart_lengths, and selfpart_quantities)
        for i in range(len(self.part_quantities))[::-1]:
            if self.part_quantities[i] == 0:
                self.part_quantities = np.delete(self.part_quantities, i, 0)
                self.part_lengths = np.delete(self.part_lengths, i, 0)
                self.part_names = np.delete(self.part_names, i, 0)

        initial_part_quantities = self.part_quantities.copy()
        initial_part_lengths = self.part_lengths.copy()
        initial_part_names = self.part_names.copy()
        ...
        if self.max_containers > self.num_parts or self.max_containers == -2:
            self.max_containers = self.num_parts
        ...
        if self.max_parts_per_nest > self.max_containers or self.max_parts_per_nest == -2:
            self.max_parts_per_nest = self.max_containers
        ...
        for i in range(self.num_parts):
            self.nested_lengths[i, 0] = self.part_lengths[i, 0] + self.spacing
        ...
        for i in range(self.num_parts):
            ...
            if self.patterns[i, i] == 0 or self.stock_length < self.left_waste + self.right_waste:
                # Zero out all outputs and exit function
                self.final_patterns = []
                self.final_allocations = 0
                self.error = 1  # error code 1 signifies that a part is too long
        ...
        self.ideal_num = (np.dot(np.transpose(self.nested_lengths),
                                 self.part_quantities) / self.nestable_length).item()
        ...
        self.pi = np.dot(self.patterns_trans_inv, self.ones_vector)
        self.worst_case = np.dot(np.transpose(self.part_quantities), self.pi)
        ...
        # max_containers)
        self.parts_sublist = np.zeros((self.num_parts - self.max_containers + 1, self.max_containers))
        ...
        # try:
        #     print(selfcurrent_sequence)
        # except AttributeError:
        #     selfcurrent_sequence = np.array(range(self.num_parts))

        # Initialize the part ordering
        self.current_sequence = np.arange(self.num_parts)

        for sub_i in range(self.num_parts - self.max_containers + 1):
            self.parts_sublist[sub_i] = np.array(range(sub_i, sub_i + self.max_containers))
            # self.parts_sublist[sub_i] = selfcurrent_sequence[sub_i:(sub_i + selfmax_containers)]

        # Iterate the part sequence to find the part ordering for which scrap is minimized
        self.part_sequence_is_optimum = 0
        self.current_part_index = 0
        self.part_quantities = initial_part_quantities[self.current_sequence].copy()

        self.part_lengths = initial_part_lengths[self.current_sequence].copy()
        self.part_names = initial_part_names[self.current_sequence].copy()

        self.parts_sublist_sorted = np.zeros((self.num_parts - self.max_containers + 1,
                                              self.max_containers))

        best_sequence = self.current_sequence.copy()

        # Run through one more time using best sequence to get best patterns

        self.part_quantities = initial_part_quantities[best_sequence].copy()

        self.part_lengths = initial_part_lengths[best_sequence].copy()
        self.part_names = initial_part_names[best_sequence].copy()
        ...
        for i in range(1, 1 + self.max_parts_per_nest)[::-1]:
            temp_max_parts_per_nest = i
            [self.required_lengths, self.allocation, self.patterns] = \
                self.column_gen(self.num_parts, temp_max_parts_per_nest, self.part_quantities,
            ...
            if self.patterns.any():
                ...
                if required_containers <= self.max_containers or temp_max_parts_per_nest == 2:
        ...
        self.part_quantities = self.part_quantities[sorted_by_start][sorted_by_end]

        self.part_names = self.part_names[sorted_by_start][sorted_by_end]
        self.part_lengths = self.part_lengths[sorted_by_start][sorted_by_end]
        ...
        containers_equals_max = containers == self.max_containers
        ...
        self.remaining_part_quantities = self.part_quantities.copy()
        ...
        for self.part_index, active_parts_row in enumerate(self.active_parts):
            ...
            if self.part_index >= (self.num_parts - self.max_containers):
                break
            if skip_to_end == 1:
                break
            if self.remaining_part_quantities[self.part_index]:
                ...
                while self.remaining_part_quantities[self.part_index]:  # Finds additional bridge pattern if needed
                    ...
                    for sublist_index, sublist in enumerate(bridge_sublist):
                        ...
                        self.branch_bound(len(parts_sublist[0]), self.max_parts_per_nest + num_extra_parts,
                                          self.part_quantities.copy(), parts_sublist, 1,
                    ...
                    remaining_length = self.stock_length - self.left_waste - self.right_waste
                            ...
                            if self.nested_lengths[p_index] < remaining_length:  # and if there is room to nest it
                                if right_bound < np.shape(self.active_parts)[1] - 2:  # and the right bound can be
                                    ...
                                    for p in best_bridge_pattern:
                                        if p[0]:
                                            num_parts_in_nest += 1
                                    if num_parts_in_nest == self.max_parts_per_nest + num_extra_parts:
                                        num_extra_parts += 1
                                    break
                                ...
                                if self.remaining_part_quantities[pattern_qty_index] < 0:
                                    ...
                                    if self.int_allocation[rightmost_pattern_index] == 0:
                                        ...
                                        while self.int_allocation[rightmost_pattern_index] == 0 or \
                                            ...
                                            # than current part)
                                            if skip_to_end == 1 or highest_in_pattern <= self.part_index:
        ...
        while self.parts_need_nested == 1:
            ...
            final_parts_sublist = np.arange(self.num_parts - self.max_containers, self.num_parts)
            if skip_to_end == 1:
                final_parts_sublist = np.arange(self.num_parts)
            final_parts_sublist = np.expand_dims(final_parts_sublist, axis=0)
            if skip_to_end == 1:
                self.branch_bound(self.num_parts, self.max_parts_per_nest + num_extra_parts,
                                  self.remaining_part_quantities, final_parts_sublist, 2)
            else:
                self.branch_bound(self.max_containers, self.max_parts_per_nest + num_extra_parts,
                                  self.remaining_part_quantities, final_parts_sublist, 2)
            ...
            for p_index, part in enumerate(downstream_parts):
                if self.nested_lengths[p_index] < remaining_length:  # check if there is room to nest it
                    if self.max_parts_per_nest + num_extra_parts <= len(downstream_parts):
                        find_new_best = 1
                        num_extra_parts += 1
                        break
        ...
        self.scrap_rates = 1 - np.dot(self.part_lengths.T, self.final_patterns) / self.stock_length
        ...
        self.max_drop_length = self.stock_length - self.right_waste
        self.max_drop_length -= np.dot(self.final_patterns.T[index_largest_drop].T, self.nested_lengths)[0]
        scrap_adjustment = self.max_drop_length / self.stock_length / np.sum(self.final_allocations)
        ...
        self.final_required_lengths_minus_drop = \
            self.final_required_lengths - self.max_drop_length / self.stock_length
        ...
        if np.size(self.final_patterns) >= 3:
            ...
            self.part_quantities = self.part_quantities[sorted_by_start]

            self.part_names = self.part_names[sorted_by_start]
            self.part_lengths = self.part_lengths[sorted_by_start]
        ...
        self.part_quantities = self.part_quantities[self.sequenced_by_networks]

        self.part_names = self.part_names[self.sequenced_by_networks]
        self.part_lengths = self.part_lengths[self.sequenced_by_networks]
        ...
        self.part_lengths = self.part_lengths[self.sorted_by_length]
        self.part_names = self.part_names[self.sorted_by_length]

        self.part_quantities = self.part_quantities[self.sorted_by_length]

        self.final_part_quantities_sorted = self.part_quantities.copy()
        self.final_part_lengths_sorted = self.part_lengths.copy()

        return self.final_patterns, self.final_allocations, self.part_names, self.part_lengths
